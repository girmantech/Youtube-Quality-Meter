...
const buildRatio = () => {
  ...
  const color = getBackgroundColor(ratio);
  newElem.style.backgroundColor = color[0];
  newElem.style.color = color[1];
  newElem.innerText = ratioText;
  newElem.style.fontWeight = '600';
  newElem.style.fontSize = '17.84px';
...
const initialize = () => {
  ...
  awaitElements(requiredElements).then(() => {
    parseLikes();
...
awaitElement(likeButtonSelector).then(() => {
